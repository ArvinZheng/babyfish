...
public final class SessionFactoryImplWrapper extends ClassWrapper {
	...
	static CurrentXSessionContext buildCurrentXSessionContext(XSessionFactoryImplementor sessionFactory) {
		...
		if ( impl == null ) {
			if (canAccessTransactionManager(sessionFactory)) {
				impl = "jta";
			}
			else {
				return null;
...
//			}
			...
			try {
				Class<?> implClass = sessionFactory.getServiceRegistry().getService( ClassLoaderService.class ).classForName( impl );
    ...
    private static boolean canAccessTransactionManager(XSessionFactoryImplementor sessionFactory) {
    	return sessionFactory.getServiceRegistry().getService( JtaPlatform.class ).retrieveTransactionManager() != null;
    }
}
